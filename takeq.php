...
// In your form submission handling section, modify the score calculation:
    if (isset($_POST["submit"])) {
        $score = 0;
        $wrong_questions = []; // Array to store question numbers answered incorrectly
        
        for ($i = 1; $i <= $count; $i++) {
            if (isset($_POST["ans$i"])) {
                if (trim($_POST["ans$i"]) == trim($answers[$i])) {
                    $score++;
                } else {
                    $wrong_questions[] = $i; // Store question number if answer is wrong
                }
            } else {
                $wrong_questions[] = $i; // Also store if no answer was selected
            }
        }
        ...
        if ($res) {
            // Convert wrong questions array to JSON for JavaScript
            $wrong_questions_json = json_encode($wrong_questions);
            ...
            echo "<script>
                ...
                // Show score and wrong questions in popup
                const scoreText = document.getElementById('scoreText');
                const thankYouMessage = document.getElementById('thankYouMessage');
                const closeBtn = document.getElementById('closeBtn');
                const wrongQuestions = $wrong_questions_json;
                
                if (scoreText && thankYouMessage && closeBtn) {
                    let scoreMessage = 'You scored $score out of $count';
                    if (wrongQuestions.length > 0) {
                        scoreMessage += '<br>Questions numbers are incorrectly: ' + wrongQuestions.join(', ');
                    } else {
                        scoreMessage += '<br>Congratulations! You got all questions correct!';
                    }
        ...
        if ($stmt) {
            ...
            echo "<p>No quiz ID provided. Please select a quiz to take.</p>";
        }
        ?>
    </div>
